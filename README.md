...
- Build and run containers locally:
  ...
  ```bash
  # backend
  docker build -t decodeXiv-backend ./backend
  docker run -p 8000:8000 -e GEMINI_API_KEY=YOUR_GEMINI_API_KEY decodeXiv-backend

  # frontend
  docker build -t decodeXiv-frontend ./frontend
  docker run -p 3000:3000 -e NEXT_PUBLIC_BACKEND_URL=http://host.docker.internal:8000 decodeXiv-frontend
  ```
